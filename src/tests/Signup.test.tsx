...
vi.mock('@/components/GoogleSignupButton', () => ({
    default: ({ onClick }: { onClick: () => void }) => (
        <button onClick={onClick} data-testid="google-signup">Mock Google</button>
    ),
}))

vi.mock('@/components/GithubLoginButton', () => ({
    default: ({ onClick }: { onClick: () => void }) => (
        <button onClick={onClick} data-testid="github-login">Mock GitHub</button>
    ),
}))
...
describe('SignupPage', () => {
    ...
    it('shows password mismatch error', async () => {
        ...
        fireEvent.change(screen.getByLabelText(/verify password/i), { target: { value: 'Wrong123' } })
        const inputButtons = screen.getAllByRole('button', { name: /sign up/i })
        fireEvent.click(inputButtons[0])
        await waitFor(() => {
